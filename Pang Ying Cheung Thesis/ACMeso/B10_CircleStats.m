...
clearvars -except keepVariables avAll compete coordinates tones colour rect I final

close all
clc

final = imcrop(I, [20 5 220 200]); 

% openfig(['Maps/cropped_BF.fig']);
...
for tt = 1:size(tones,2);
    ...
    if isempty(row1)==0; % Not empty
        ...
        if tt == size(tones,2); % last tone have no next tone; no vector
            ...
            ylim([0 100]);
            xlim([0 100]);
            title ([num2str(tt*4), ' kHz'],'FontSize',24,'FontWeight','Bold');
            ...
            if isempty(row2)==0; % There is BF of next tone
                ...
                figure(2);
                hold on
                quiver(Y_each,X_each,V_each,U_each,5,'color',colour(tt,:),'LineWidth',1,'MaxHeadSize',50) % direction of each BF
                set(gca,'Ydir','reverse'); 
                set(gca,'color','k');
                ...
                quiver(y_BF,x_BF,compy,compx,10,'r','LineWidth',2,'MaxHeadSize',2) % resultant direction
...
% quiver (Y,X,V,U,2,'color','#C7C6C1','LineWidth',1); % Hexadecimal Color Code
 quiver (Y,X,V,U,1,'color','k','LineWidth',0.5,'MaxHeadSize',100); % Hexadecimal Color Code
set(gca,'Ydir','reverse');
set(gca,'color','k');
