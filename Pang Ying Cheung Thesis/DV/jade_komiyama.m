...
stack = imresize(stack,0.45); % Both dimensions must be in the 100s or less or else get an error about not enough space to run jader.
% stack(:,:,12001:end) = [];
...
while variance <= 95 % While-loop keeps going as long as the statment holds true
    variance = sum(explained(1:(1*w),1));
    w = w +1;
end
disp(['number of PC explaining >95% of variance = ' num2str(w)])

q = 35; % number of the components to use
...
figure
set(gcf, 'color','w')
 tiledlayout(5,7,'TileSpacing','Compact')      
...
ModeICA = (B*ModePCA')'; % multiply separating matrix to the PC coefficients 
...
if HowMany >0;
    ...
    for jj = 1:size(unwanted, 2);
        ...
        title(titlename)
        colormap turbo
        saveas(gcf,['PCA-ICA analysis/PCA-ICA/Unwanted IC/',filename(1:end-4),'_',titlename])
    end
    close
else HowMany == 0;
    ;
