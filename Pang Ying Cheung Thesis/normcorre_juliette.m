...
options_r = NoRMCorreSetParms('d1',d1-bound,'d2',d2-bound,'bin_width',200,'max_shift',50,'iter',2,'correct_bidir',false);
% Original iter = 1; 
% origina max_shift = 100;
...
[M1,shifts1,template1] = normcorre_batch(Y(bound/2+1:end-bound/2,bound/2+1:end-bound/2,:),options_r); % register filtered data
    % exclude boundaries due to high pass filtering effects
Mr = apply_shifts(Yf,shifts1,options_r,bound/2,bound/2); % apply shifts to full dataset
...
% 
% [M2,shifts2,template2] = normcorre_batch(Y(bound/2+1:end-bound/2,bound/2+1:end-bound/2,:),options_nr,template1); % register filtered data
% Mpr = apply_shifts(Yf,shifts2,options_nr,bound/2,bound/2);  % apply the shifts to the removed percentile
% 
...
% end
close all
%clearvars -except keepVariables Mr iFile filelist All GP shifts1 options_r bound
% % tic
 %Mr = apply_shifts(Yf,shifts1,options_r,bound/2,bound/2); % apply shifts to full dataset
