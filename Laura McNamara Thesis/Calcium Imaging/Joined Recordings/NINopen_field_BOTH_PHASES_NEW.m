%% Neuronal firing analysis in open field in joined recordings
...
for iAnimal = 1:2:nAnimals; %%%%%%%%%%%%%%
    ...
    qALL = [];
    joinsize = 1:1;  %% 1:1, 1:2 or 1:3 depending on how many recordings are available
    animal=animals(iAnimal).name;
    folder=animal;
    cd(folder)

    %% loads in first animal of the joined recordings
    ...
    if size(qALL,1)<max(p1scope(:,4))   %Expands location array to match scope array, disable loop if scope recording was truncated
        qALL(end+1: max(p1scope(:,4)),:)=0;
    end
    qALL=qALL(1:max(p1scope(:,4)),:);

    cd ..

    for i = joinsize
        iAnimalnext = iAnimal + i;

        animal=animals(iAnimalnext).name;
        folder=animal;
        cd(folder)
        ...
        if size(q,1)<max(p2scope(:,4))   %Expands location array to match scope array, disable loop if scope recording was truncated
            q(end+1: max(p2scope(:,4)),:)=0;
        end
        q=q(1:max(p2scope(:,4)),:);
    ...
    animal=animals(iAnimal+max(joinsize)).name;
    folder=animal;
    cd(folder)
...
clear p1behave p1scope p2behave p2scope scope behave q qALL

for iAnimal = 2:2:nAnimals; %%%%%%%%%%%%%%%%
    iAnimal
    animals=dir;
    animals(1:2,:)=[];
    animals = natsortfiles(animals);
...
%     end
    ...
    save behave_joined 'behave_joined'  %[Adds c7 = whether animal was in int or ext for behavioural frame], c8 = number of peaks fired for a particular behavioural frame
