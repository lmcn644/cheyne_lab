...
for iAnimal = 1%:nAnimals;
    ...
    %         bits=[];
        ...
        if isfolder('Correctedi2\')  %'Correctedi2\'
            if isfile('bitsi2.mat') %isfile('bitsi2.mat')
             
                    load bitsi2.mat  %load bitsi2.mat


            end
            if not(isfile('bits.mat'))
                %% load and save bits
                folder2='Correctedi2\';  %'Correctedi2\';
                filelist=dir(folder2);
                filelist(1:2,:)=[];
                filelist = natsortfiles(filelist);
                nChunks=size(filelist,1);
                cd('Correctedi2') %'Correctedi2'
    ...
    nChunks=5; %MUST MATCH NUMBER OF CHUNKS IN /Correctedi2
    All=[];
    %%
        animal=animals(iAnimal).name
        folder=animal;
