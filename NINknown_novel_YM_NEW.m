...
%     end
    ...
    framerate_bh = max(behave(:,1))/(max(behave(:,2))/1000); %Framerate of behavioural recording

    t_q=sum(quadrant,1)/framerate_bh;
    t_q(1,4) = (size(behave,1) - sum(sum(quadrant,1)))/framerate_bh;  %puts time spent in centre in c4 of t_q
    ...
    for iPeak=1:nPeaks
        ...
        if  ismember(frame, scope(:,1)) == 0
            frame = frame+1; %Contingency in case scope frames don't begin at '1'
        end
        %if frame<size(scope(:,1),1)
        temp=scope(frame-1:frame+1,6); %Takes small range to index for equivalent behavioural frame where peak is fired
        if nansum(temp)==0
            temp=scope(frame-2:frame+2,6);
        end
    ...
    %%
    b_framerate= max(behave(:,1))/(max(behave(:,2))/1000); %frame rate of behavioural recording
    ...
    YM(1,2)=YM(1,1)./b_framerate;    % 2 seconds spent in left arm
    ...
    YM(2,2)=YM(2,1)./b_framerate; % 2 seconds spent in the right arm
    ...
    YM(3,2)=YM(3,1)./b_framerate; % 2 seconds spent in the familiar arms
    ...
    YM(4,2)=YM(4,1)./b_framerate; % 2 seconds spent in the novel arm
    ...
    YM(5,2)=YM(5,1)./b_framerate; % 2 seconds spent in the middle
